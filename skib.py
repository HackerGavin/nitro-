    ...
    async with aiohttp.ClientSession() as session:
        for _ in range(codes_to_generate):
            ...
            if await check_code_validity(session, generated_code):
                ...
                safe_send_to_webhook(webhook_url, f"Valid code found: {valid_code_link}")  # Send valid code to webhook
            else:
                update_counts(0, 1)
            ...
            if total_codes_processed % 100000 == 0:
                with lock:
                    safe_send_to_webhook(webhook_url, f"@everyone Invalid Codes: {invalid_count} | Valid Codes: {valid_count}")
...
def safe_send_to_webhook(webhook_url, message):
    """Wraps the async send_to_webhook to be called from a thread."""
    asyncio.run_coroutine_threadsafe(send_to_webhook(webhook_url, message), loop)
...
def check_for_view_input(webhook_url):
    """Check for user input to send code status to webhook."""
    while True:
        user_input = input()  # Wait for input from the user
        if user_input.lower() == 'codes':
            with lock:
                if valid_count > 0:
                    valid_code_links = "\n".join(valid_codes_list)
                    message = (
                        f"@everyone\n"
                ...
                safe_send_to_webhook(webhook_url, message)
...
if __name__ == "__main__":
    ...
    try:
        ...
        if total_codes_to_generate < 1:
            ...
            threading.Thread(target=check_for_view_input, args=(webhook_url,), daemon=True).start()

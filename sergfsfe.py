    ...
    async with aiohttp.ClientSession() as session:
        for _ in range(codes_to_generate):
            ...
            if total_codes_processed % 10000 == 0:
                print(f"{Fore.LIGHTYELLOW_EX}Sending status update...{Style.RESET_ALL}")  # Debugging line
...
async def send_to_webhook(webhook_url, message):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(webhook_url, json=data) as response:
            if response.status != 204:  # Status 204 means no content, which is expected
                print(f"{Fore.RED}Failed to send message to webhook: {response.status} {await response.text()}{Style.RESET_ALL}")
            else:
                print(f"{Fore.GREEN}Message sent to webhook successfully.{Style.RESET_ALL}")  # Debugging line
...
async def send_status_update(webhook_url):
    ...
    with lock:
        if valid_count > 0:
            ...
            valid_codes_message = "No valid codes found."
...
def input_listener(webhook_url):
    """Listen for user input to send to the webhook."""
    while True:
        user_input = input()  # Wait for input from the user
        if user_input.lower() == 'test':
            asyncio.run(send_to_webhook(webhook_url, "test"))
...
if __name__ == "__main__":
    ...
    try:
        ...
        if total_codes_to_generate < 1:
            ...
            # Start the status report and input listener threads
            threading.Thread(target=status_report, daemon=True).start()
            threading.Thread(target=input_listener, args=(webhook_url,), daemon=True).start()

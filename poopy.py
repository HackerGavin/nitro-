    ...
    async with aiohttp.ClientSession() as session:
        for _ in range(codes_to_generate):
            ...
            if await check_code_validity(session, generated_code):
                ...
                await send_to_webhook(webhook_url, valid_code_link)  # Send valid code to webhook
            else:
                update_counts(0, 1)
            ...
            if total_codes_processed % 10000 == 0:
                with lock:
                    await send_to_webhook(webhook_url, f"@everyone Invalid Codes: {invalid_count} | Valid Codes: {valid_count}")
